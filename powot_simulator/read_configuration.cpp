...
void powotsimulator::read_configuration(void){
    ...
    case MODEL_BINARY:
        bin_model_aux_xml_file = bin_model_path;
        bin_model_aux_xml_file += '/';
        bin_model_aux_xml_file += bin_model_file.section('.', 0, 0);
        bin_model_aux_xml_file += ".xml";
        qDebug()<<"(powotsimulator) Auxiliary model XML file: "<<bin_model_aux_xml_file;
        QFile file_check(bin_model_aux_xml_file);
        if(!file_check.exists()){
            err_message.display_error(MODEL_XML_NOT_LOADED);
            qDebug()<<"(powotsimulator) Aborting ...";
            exit(EXIT_FAILURE);
        }
